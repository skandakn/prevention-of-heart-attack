...
import type { RestIntent, FitRestISIContext } from "@/lib/fit-rest/types";
...
const QUICK_PROMPTS = [
  {
    label: "Sleep tips",
    text: "Give me 5 practical sleep tips based on my sleep goals and challenges.",
    intent: "recommendations" as const,
  },
  {
    label: "Wind-down routine",
    text: "Create a calming bedtime routine based on my schedule and preferences.",
    intent: "bedtime_routine" as const,
  },
  {
    label: "Improve my sleep",
    text: "How can I improve my sleep quality based on my current patterns?",
  },
  {
    label: "Why these suggestions?",
    text: "Why are you suggesting these sleep and recovery recommendations for me?",
    ...
    <div className={cn("flex gap-2.5 items-start", isUser && "flex-row-reverse")}>
      ...
      <div
        className={cn(
          ...
          isUser ? "bg-navy-900 text-white" : "bg-indigo-100 text-indigo-700"
        )}
      >
...
export function RestAIChat() {
  ...
  // ISI context not available for Rest AI
  ...
  async function sendMessage(
    ...
    try {
      ...
      if (data.success !== true) {
        throw new Error(
          data.error || "The Sleep Coach could not generate a response. Please try again."
        );
      }

      if (
        typeof data.responseText !== "string" ||
        data.responseText.trim().length === 0
      ) {
        throw new Error(
          "The Sleep Coach returned an empty response. Please try again."
        );
      }

      const assistantMsg: RestMessage = {
        id: `msg_${Date.now()}_a`,
        role: "assistant",
      ...
      <CardContent className="flex flex-1 flex-col gap-3 overflow-hidden p-4 pt-0">
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
          ...
          {isLoading && (
            <div className="flex gap-2.5 items-start">
              <div className="shrink-0 flex h-7 w-7 items-center justify-center rounded-full bg-indigo-100 text-indigo-700">
        ...
        <div className="flex gap-2 pt-1 shrink-0">
          <input
            ...
            placeholder={
              isLoading
                ? "Sleep Coach is thinking…"
                : "Ask about sleep or rest…"
            }
            disabled={isLoading}
